...
public:
    ...
    bool initialize(FeatureContext const& context) override {
        ...
        if (XR_SUCCEEDED(xrGetSystem(mContext.instance, &systemInfo, &systemId)) &&
                XR_SUCCEEDED(
                        xrGetSystemProperties(mContext.instance, systemId, &systemProperties)) &&
...
private:
    ...
    bool createHand(uint32_t hand) {
        ...
        state.indexBuffer = IndexBuffer::Builder()
                                    ...
                                    .build(engine);
        state.indexBuffer->setBuffer(engine,
                { copy(indices.data(), indicesSize), indicesSize, free });
        ...
        RenderableManager::Builder(1)
                ...
                .geometry(0, RenderableManager::PrimitiveType::TRIANGLES, state.outlineVertexBuffer,
                        state.outlineIndexBuffer, 0, indexCount)
                .priority(5)
                .culling(false)
                .castShadows(false)
        ...
        XRLOG("hand meshes: %s hand has %u vertices, %u indices, %u joints, %.1f mm outline",
                kHandNames[hand], vertexCount, indexCount, jointCount, kOutlineThickness * 1000.0f);
        return true;
    }
...
std::unique_ptr<Feature> createHandMeshes() { return std::make_unique<HandMeshes>(); }
