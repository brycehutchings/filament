...
//
// Filament renders both eyes in a single multiview pass straight into the OpenXR color swapchain
// images: the XR runtime creates the images, the sample imports each one as a Texture and wraps it
// in a RenderTarget, and the two-layer swapchain turns that into a multiview render pass.
...
public:
    ...
    // Copies one presented image back to the host to verify both multiview layers were written.
    void dumpFrame(XrSwapChain const& swapChain, uint32_t colorIndex, char const* prefix) const;
...
private:
    ...
    std::vector<uint8_t> readBack(VkImage image, VkImageAspectFlags barrierAspect,
            VkImageAspectFlags copyAspect, VkImageLayout layout, VkExtent2D extent, uint32_t layers,
            uint32_t bytesPerPixel) const {
        VkDevice const device = getDevice();
        VkDeviceSize const size =
                VkDeviceSize(extent.width) * extent.height * bytesPerPixel * layers;
    ...
    void dumpFrameImpl(XrSwapChain const& swapChain, uint32_t const colorIndex,
        ...
        for (uint32_t layer = 0; layer < layers; ++layer) {
            ...
            XRLOG("frame dump: eye %u mean luma %.1f", layer, luma / double(pixelsPerLayer));
            writePpm(std::string(prefix) + "_eye" + std::to_string(layer) + ".ppm", data, extent);
        }
        if (layers > 1) {
...
inline void XrVulkanPlatform::dumpFrame(XrSwapChain const& swapChain, uint32_t const colorIndex,
        char const* prefix) const {
    dumpFrameImpl(swapChain, colorIndex, prefix);
}
...
public:
#if defined(__ANDROID__)
    HelloXr(Config const& config, android_app* app)
            : mConfig(config),
              mApp(app) {
...
#else
    explicit HelloXr(Config const& config)
            : mConfig(config) {
        addFeatures();
    }
#endif
    ...
    bool initialize() {
        return createXrInstance() && createVulkanContext() && createSession() &&
               mControllerInput.initialize(mXrInstance, mSession) && createSwapChains() &&
               createEngine() && createScene() && initializeFeatures();
    }
...
private:
    ...
    bool initializeFeatures() {
        helloxr::FeatureContext const context{ mXrInstance, mSession, mAppSpace, mViewSpace,
            mEngine, mScene, mMaterial, mConfig.dumpFrame != 0 ? mConfig.dumpPrefix : std::string(),
            &mControllerInput, &mJetpackUi, &mQuadLayer };
        for (auto& feature: mFeatures) {
            if (!feature) {
                continue;
            }
    ...
    bool createXrInstance() {
        ...
        for (auto& feature: mFeatures) {
            ...
            if (satisfied) {
                ...
                XRLOG("%s disabled: the runtime is missing one of its extensions", feature->name());
                feature.reset();
            }
        }
        ...
        if (xrCheck(xrGetSystemProperties(mXrInstance, mSystemId, &systemProps),
            ...
            mQuadLayer.configure(mConfig.quadLayer, remainingLayerCount, mConfig.msaa,
                    mConfig.quadMsaa);
        }
        ...
        XRLOG("view configuration: %ux%u per eye (render scale %.2f)", mEyeWidth, mEyeHeight,
                mConfig.renderScale);
    ...
    static bool hasInstanceLayer(char const* name) {
        ...
        return std::any_of(layers.begin(), layers.end(),
                [name](VkLayerProperties const& l) { return strcmp(l.layerName, name) == 0; });
    }
    ...
    bool createSession() {
        ...
        spaceInfo.referenceSpaceType =
                hasLocal ? XR_REFERENCE_SPACE_TYPE_LOCAL : XR_REFERENCE_SPACE_TYPE_STAGE;
    ...
    bool createSwapChains() {
        ...
        // The sample writes linear values directly into the color target.
    ...
    bool createXrSwapChain(XrVulkanPlatform::XrSwapChain* out, uint32_t width, uint32_t height,
        ...
        if (!xrCheck(xrCreateSwapchain(mSession, &createInfo, &out->color),
                ...
                !enumerateSwapChainImages(out->color, &colorImages)) {
            return false;
        }
        ...
        XRLOG("swapchain: %ux%u, %u images, %u layers, color format %d", width, height,
                uint32_t(colorImages.size()), arraySize, int(mColorFormat));
        return true;
    }
    ...
    bool createEngine() {
        ...
        // ordinary attachments instead of through the swapchain path. Its transparency flag keeps
        // alpha available for passthrough and composition layers.
    ...
    // Wrap each XR image as a Texture. RenderTargets are created lazily.
    bool createRenderTargets(XrVulkanPlatform::XrSwapChain const& swapChain, RenderTargetSet* out) {
        ...
        Texture::Sampler const sampler =
                layers > 1 ? Texture::Sampler::SAMPLER_2D_ARRAY : Texture::Sampler::SAMPLER_2D;

        out->layers = layers;
        out->colors.reserve(bundle.colors.size());
        for (VkImage const image: bundle.colors) {
            Texture* const color = Texture::Builder()
                                           .import(reinterpret_cast<intptr_t>(image))
        ...
        out->pairs.resize(out->colors.size());
        XRLOG("wrapped %zu color XR images", out->colors.size());
        return true;
    }
    ...
    filament::RenderTarget* getRenderTarget(RenderTargetSet& targets,
        ...
        RenderTarget*& target = targets.pairs[images.color];
        if (target) {
            return target;
        }
    ...
    bool createScene() {
        ...
        if (!mQuadLayer.initialize(mEngine, mScene, mPassthroughActive)) {
            return false;
        }
        return true;
    }
    ...
    void renderFrame() {
        ...
        if (frameState.shouldRender) {
            ...
            // OpenXR composites back-to-front, so submit quads before the projection layer.
            if (quadRendered) {
    ...
    void queueSwapChainRelease(XrVulkanPlatform::XrSwapChain& swapChain,
        ...
        mEngine->queueDriverCommand([this, &swapChain, images, dump] {
            if (dump) {
                mPlatform.dumpFrame(swapChain, images.color, mConfig.dumpPrefix.c_str());
            }
    ...
    bool renderLayer(XrTime displayTime, FrameSubmission& submission) {
        ...
        for (uint32_t i = 0; i < kEyeCount; ++i) {
            ...
            view.subImage.imageRect = { { 0, 0 }, { int32_t(mEyeWidth), int32_t(mEyeHeight) } };
    ...
    bool renderQuadLayer(helloxr::QuadLayer::Submission* submission) {
        ...
        bool const rendered = mQuadLayer.render(getRenderTarget(mQuadRenderTargets, images),
                mFilamentSwapChain, mAppSpace, mQuad.color, submission);
...
void printUsage() {
    XRLOG("helloxr: renders a Filament scene through OpenXR using Vulkan multiview.\n"
          ...
          "  --dump-frame=N    read frame N back and report per-eye color stats\n"
