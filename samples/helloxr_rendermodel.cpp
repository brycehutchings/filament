...
public:
    ...
    bool initialize(FeatureContext const& context) override {
        ...
        auto* provider = gltfio::createUbershaderProvider(mContext.engine, UBERARCHIVE_DEFAULT_DATA,
    ...
    bool loadModel(uint32_t hand) {
        ...
        if (!mContext.dumpPrefix.empty()) {
            std::string const path =
                    mContext.dumpPrefix + "_controller_" + (hand == 0 ? "left" : "right") + ".glb";
...
std::unique_ptr<Feature> createRenderModels() { return std::make_unique<RenderModels>(); }
