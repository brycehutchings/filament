...
public:
    ...
    bool initialize(FeatureContext const& context) override {
        mContext = context;
        bool const hasJetpack = mContext.jetpackUi != nullptr &&
                                mContext.jetpackUi->isEnabled();
        bool const hasFixedQuad = mContext.quadLayer != nullptr &&
                                  mContext.quadLayer->isEnabled();
        ...
        mVertices->setBufferAt(engine, 0,
                { vertices, sizeof(QUAD_VERTICES), releaseVertices });
        ...
        updateTransforms();
        XRLOG("quad depth proxies: Jetpack %s, fixed quad %s",
                hasJetpack ? "enabled" : "disabled",
...
private:
    ...
    bool loadMaterial() {
        ...
        mMaterial = Material::Builder()
                            .package(RESOURCES_XRDEPTHPROXY_DATA,
                                    RESOURCES_XRDEPTHPROXY_SIZE)
    ...
    void updateTransforms() {
        ...
        if (!mJetpackProxy.isNull()) {
            ...
            transforms.setTransform(transforms.getInstance(mJetpackProxy),
                    mat4f(poseToMat4(pose)) * mat4f::scaling(float3{
                            JetpackUiLayer::WIDTH_METERS,
...
std::unique_ptr<Feature> createQuadDepthProxies() {
    return std::make_unique<QuadDepthProxies>();
}
