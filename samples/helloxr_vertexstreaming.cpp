...
public:
    ...
    bool initialize(FeatureContext const& context) override {
        ...
        XRLOG("vertex streaming: %u CPU vertices, %u triangles", kVertexCount, kIndexCount / 3);
        return true;
    }
...
private:
    ...
    void streamVertices(float seconds) {
        ...
        for (uint32_t stack = 0; stack <= kStackCount; ++stack) {
            ...
            for (uint32_t slice = 0; slice <= kSliceCount; ++slice) {
                float const azimuth = kTwoPi * float(slice) / float(kSliceCount);
                float const travelingWave =
                        std::sin(11.0f * polar - 3.8f * seconds +
        ...
        for (uint32_t stack = 0; stack <= kStackCount; ++stack) {
            ...
            for (uint32_t slice = 0; slice <= kSliceCount; ++slice) {
                ...
                float3 const alongSlice = mPositions[vertexIndex(stack, nextSlice)] -
                                          mPositions[vertexIndex(stack, previousSlice)];
                float3 const alongStack = mPositions[vertexIndex(nextStack, slice)] -
                                          mPositions[vertexIndex(previousStack, slice)];

                float3 tangent = normalizedOr(alongSlice, fallbackTangent);
                float3 normal =
                        stack == 0 || stack == kStackCount
                                ? mDirections[index]
                                : normalizedOr(cross(alongSlice, alongStack), mDirections[index]);
                if (dot(normal, mDirections[index]) < 0.0f) {
                    normal = -normal;
                }
                float3 const bitangent =
                        normalizedOr(cross(normal, tangent), float3{ 0.0f, 1.0f, 0.0f });
    ...
    void updateAppearance(float seconds) {
        ...
        mAuraMaterialInstance->setParameter("fillColor", RgbType::LINEAR, primary * 0.35f);
    ...
    void updateTransforms(float seconds) {
        mat4f const rotation =
                mat4f::rotation(seconds * 0.18f, normalize(float3{ 0.2f, 1.0f, 0.1f }));
...
std::unique_ptr<Feature> createVertexStreaming() { return std::make_unique<VertexStreaming>(); }
