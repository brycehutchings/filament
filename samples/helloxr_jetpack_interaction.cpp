...
void releaseVertices(void* buffer, size_t, void*) { delete[] static_cast<Vertex*>(buffer); }

void releaseIndices(void* buffer, size_t, void*) { delete[] static_cast<uint16_t*>(buffer); }
...
std::vector<float2> roundedRectangle(float halfWidth, float halfHeight, float radius) {
    ...
    for (uint32_t corner = 0; corner < 4; ++corner) {
        for (uint32_t segment = 0; segment < CORNER_SEGMENTS; ++segment) {
            float const angle =
                    (float(corner) + float(segment) / float(CORNER_SEGMENTS)) * PI * 0.5f;
...
public:
    ...
    void update(XrTime displayTime) override {
        ...
        if (mDragging) {
            if (pressed) {
                ...
                if (XR_SUCCEEDED(xrLocateSpace(mContext.viewSpace, mContext.appSpace, displayTime,
                            &head)) &&
                        (head.locationFlags & REQUIRED) == REQUIRED) {
                    float3 const headPosition = { head.pose.position.x, head.pose.position.y,
                ...
                panelPose.position = { panelPosition.x, panelPosition.y, panelPosition.z };
        ...
        bool const panelPlaneHit =
                intersectPlane(0.0f, &panelPoint, &panelLocalPoint, &panelDistance);
        bool const glassPlaneHit =
                intersectPlane(-GLASS_OFFSET, &glassPoint, &glassLocalPoint, &glassDistance);
        ...
        if (glassHit) {
            mGlassMaterialInstance->setParameter("hitPoint",
                    float2{
        ...
        transforms.setTransform(transforms.getInstance(mRay.entity),
                aimTransform *
                        mat4f::scaling(float3{ 0.003f, 0.003f, std::min(rayDistance, 3.0f) }));
        ...
        if (uiHit) {
            transforms.setTransform(transforms.getInstance(mCursor.entity),
                    worldFromPanel *
                            mat4f::translation(float3{ panelLocal.x, panelLocal.y, 0.006f }) *
        ...
        if (glassHit) {
            transforms.setTransform(transforms.getInstance(mGlow.entity),
                    worldFromPanel *
                            mat4f::translation(
                                    float3{ glassLocal.x, glassLocal.y, -GLASS_OFFSET + 0.003f }) *
    ...
    void terminate() override {
        ...
        for (MaterialInstance** instance: { &mGlassMaterialInstance, &mBorderMaterialInstance,
                 &mRayMaterialInstance, &mCursorMaterialInstance, &mGlowMaterialInstance }) {
            if (*instance != nullptr) {
...
private:
    ...
    bool createMesh(Mesh* mesh, std::vector<Vertex> const& vertices,
            std::vector<uint16_t> const& indices, MaterialInstance* material, Box const& bounds) {
        ...
        RenderableManager::Builder(1)
                .boundingBox(bounds)
                .material(0, material)
                .geometry(0, RenderableManager::PrimitiveType::TRIANGLES, mesh->vertices,
                        mesh->indices)
                .culling(false)
                .castShadows(false)
    ...
    bool createRoundedRing(Mesh* mesh, MaterialInstance* material, float outerHalfWidth,
            float outerHalfHeight, float outerRadius, float innerHalfWidth, float innerHalfHeight,
            float innerRadius) {
        ...
        for (size_t point = 0; point < outer.size(); ++point) {
            ...
            indices.insert(indices.end(),
                    { outerCurrent, outerNext, innerCurrent, outerNext, innerNext, innerCurrent });
        }
        if (!createMesh(mesh, vertices, indices, material,
                    { { 0.0f, 0.0f, 0.0f }, { outerHalfWidth, outerHalfHeight, 0.001f } })) {
            return false;
        }
        return true;
    }
    ...
    bool createFrame() {
        return createRoundedRing(&mFrame, mGlassMaterialInstance, OUTER_HALF_WIDTH,
                OUTER_HALF_HEIGHT, OUTER_RADIUS, INNER_HALF_WIDTH, INNER_HALF_HEIGHT, INNER_RADIUS);
    }
    ...
    void updatePanelTransforms(XrPosef const& pose) {
        mat4f const transform =
                mat4f(poseToMat4(pose)) * mat4f::translation(float3{ 0.0f, 0.0f, -GLASS_OFFSET });
        auto& transforms = mContext.engine->getTransformManager();
    ...
    bool createRay() {
        ...
        for (uint32_t segment = 0; segment < RAY_SEGMENTS; ++segment) {
            ...
            indices.insert(indices.end(),
                    { nearCurrent, farCurrent, nearNext, nearNext, farCurrent, farNext });
        }
    ...
    bool createDisk(Mesh* mesh, MaterialInstance* material) {
        ...
        for (uint32_t segment = 0; segment < CIRCLE_SEGMENTS; ++segment) {
            ...
            indices.insert(indices.end(),
                    { 0, uint16_t(segment + 1), uint16_t((segment + 1) % CIRCLE_SEGMENTS + 1) });
        }
