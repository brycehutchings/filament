...
bool ControllerInput::initialize(XrInstance instance, XrSession session) {
    ...
    auto const createAction = [&](char const* name, char const* localizedName, XrActionType type,
                                      XrAction* action) {
    ...
    if (mHandInteractionSupported) {
        ...
        for (uint32_t hand = 0; hand < HAND_COUNT; ++hand) {
            ...
            xrStringToPath(instance, (prefix + "/input/grip/pose").c_str(), &grip);
            xrStringToPath(instance, (prefix + "/input/aim/pose").c_str(), &aim);
            xrStringToPath(instance, (prefix + "/input/aim_activate_ext/value").c_str(), &activate);
...
void ControllerInput::update(XrTime displayTime, XrSpace appSpace) {
    ...
    for (uint32_t hand = 0; hand < HAND_COUNT; ++hand) {
        ...
        auto const locate = [&](XrSpace space, XrPosef* pose) {
            ...
            bool const valid =
                    XR_SUCCEEDED(xrLocateSpace(space, appSpace, displayTime, &location)) &&
        ...
        if (XR_SUCCEEDED(xrGetCurrentInteractionProfile(mSession, state.path, &profile)) &&
            ...
            if (profile.interactionProfile != XR_NULL_PATH) {
                ...
                uint32_t length = 0;
                if (XR_SUCCEEDED(xrPathToString(mInstance, profile.interactionProfile, sizeof(path),
                            &length, path))) {
