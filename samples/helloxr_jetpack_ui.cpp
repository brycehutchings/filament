...
constexpr char const* PANEL_CLASS = "com.google.android.filament.openxr.ComposePanelSurface";
...
struct JetpackUiLayer::Impl {
    ...
    jclass loadPanelClass(JNIEnv* env) const {
        ...
        jclass panelClass =
                reinterpret_cast<jclass>(env->CallObjectMethod(classLoader, loadClass, name));
    ...
    bool attach(jobject surface) const {
        ...
        if (panelClass != nullptr) {
            ...
            if (attachMethod != nullptr) {
                env->CallStaticVoidMethod(panelClass, attachMethod, activity, surface, PIXEL_WIDTH,
                        PIXEL_HEIGHT);
            }
    ...
    void detach() const {
        ...
        if (panelClass != nullptr) {
            jmethodID detachMethod =
                    env->GetStaticMethodID(panelClass, "detach", "(Landroid/app/Activity;)V");
...
void JetpackUiLayer::requestExtensions(bool const requested,
        std::function<bool(char const*)> const& supports, std::vector<char const*>* extensions) {
...
XrPosef JetpackUiLayer::getPose() const noexcept { return mImpl->layer.pose; }

void JetpackUiLayer::setPose(XrPosef const& pose) noexcept { mImpl->layer.pose = pose; }

bool JetpackUiLayer::isEnabled() const noexcept { return mImpl->enabled; }
...
void JetpackUiLayer::requestExtensions(bool requested, std::function<bool(char const*)> const&,
...
bool JetpackUiLayer::initialize(XrInstance, XrSession, XrSpace) { return true; }
void JetpackUiLayer::terminate() noexcept {}
